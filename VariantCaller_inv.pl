###############
# DevRO: Variant Caller: INV Script 
##
##
## @copyright_shumaila.sayyab@gmail.com GNU @version DevRO_v5.0.01
##
##
#########################################
...
while($reg=<region>)
{
	...
	while($st_pos <= ($chr_size))
	{
		if($st_pos >0 and $end_pos <$chr_size)
		{
			...
			@DEL_Reads=();@DEL_MateDistances=();
			@INS_Reads=();@INS_MateDistances=();
			...
			foreach $line (@align_st)
			{
				...
				{	
						 ...
						 { 
							...
							if($mateid eq "=" and $mate_mapped==1 and $read_mapped==1 and ($insert >$InsertRange or $insert < -$InsertRange)) 
							{
				                #print "DEL side $side\n";
				                $del++;
								$mate_dist=$mate_st+$ReadLength_default-1; # 50 is added here assuming read length of 50 and going in the end of read
							    push (@DEL_MateDistances,"$mate_dist");
				                
				             }else{}
				             if($mateid eq "=" and $mate_mapped==1 and $read_mapped==1 and (($insert <$InsertRangeMin and $insert > 0) or ($insert > -$InsertRangeMin and $insert < 0))) 
				             {
				             	$ins++;
				                #print "INS side $side\n";
 								$mate_dist2=$mate_st+$ReadLength_default-1; # 50 is added here assuming read length of 50 and going in the end of read
							    push (@INS_MateDistances,"$mate_dist2");
				             }else{}
							if($read_strand_plus == $mate_strand_plus and $mate_mapped==1 and $read_mapped==1 and $mateid eq "=" and $insert >=$InsertRange)
							{
								...
								$invy_st_F++;print OUTINVFILE "F1F1	".$line;								
				  ...
				  if($mateid eq "=" and $mate_mapped==1 and $read_mapped==1 and (($insert < $InsertRangeMin and $insert > 0) or ($insert > -$InsertRangeMin and $insert < 0)))
				  {
						 	...
						 	if($read_strand_plus == $mate_strand_plus and $mate_mapped==1 and $read_mapped==1 and $mateid eq "=" and $insert >=$InsertRange)
						 	{
						 		...
						 		$invy_st_R++;print OUTINVFILE "R1R1	".$line;					 
							    push (@MateDistances,"$mate_distance");
			...
			$median_dist=int(median(@MateDistances));
			
			$median_del=int(median(@DEL_MateDistances));
			$median_ins=int(median(@INS_MateDistances));
				...
				{	
									#DeviantsMate($read_strand_plus);
											 ...
											 { 
												...
												{
													$invy_end_F++; print OUTINVFILE "F2F2	".$lineEnd;
												}else{}
												if($mate_mapped==0 and $read_mapped==1)
												{
	...
	if((($flg1 ==145 and $insert1 >$InsertRange) or ($flg1 ==81 and $insert1 >$InsertRange) or ($flg1 ==97 and $insert1 <$InsertRangeMin) or ($flg1 ==161 and $insert1 <$InsertRangeMin)) and ($mateid1 eq "=" and $mate_mapped==1 and $read_mapped==1))
	{
											 ...
											 { 
												...
												if($mateid1 eq "=")
												{
											 	...
											 	if($read_strand_plus == $mate_strand_plus and $mate_mapped==1 and $read_mapped==1 and $mateid1 eq "=" and $insert1 > $InsertRange)
											 	{
											 		$invy_end_R++;print OUTINVFILE "R2R2	".$lineEnd;
											 	}else{}
											 	if($mate_mapped==0 and $read_mapped==1)
												{
	...
	if((($flg1 == 145 and $insert1 >$InsertRange) or ($flg1 ==81 and $insert1 >$InsertRange) or ($flg1 ==97 and $insert1 <$InsertRangeMin) or ($flg1 ==161 and $insert1 <$InsertRangeMin)) and ($mateid1 eq "=" and $mate_mapped==1 and $read_mapped==1))
	{
		...
		if($median_dist >0)
		{
			...
			#display(@DEL_Reads); ## hashed to syncronize with dup script...
			#display(@INS_Reads); ## hashed to syncronize with dup script...
			
			#print OUT $median_del."\t".$median_ins."\t".$median_dist_dup; ## hashed to syncronize with dup script...
			print OUT "\n";		
		}else
		{#print "sorry_bin"."\n";
		}## end of printing if
...
sub GetCurDateTime {
	## adapted from some other software
...
## helping commands:::
##$num_unmapped_pairs=`samtools view $new_blacklist -q $MapQ -f8 -F 1536 -c $INPUT_BAM $target_drp`; ## singleton reads
###

########Insert size of library and getting the mean and standard deviation
###
...
##samtools view -f 16 -F 1568 BAMS_input/CL4_sample.dedup.bam chr2|awk '{OFS="\t"}{if(($7 ~ /=/)&&($4<$8)&&($9>0)&&($9>4000)){print "",$0}}'|head
